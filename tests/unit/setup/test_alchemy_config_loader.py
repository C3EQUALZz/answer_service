...
def test_rejects_pool_recycle_below_minimum() -> None:
    with pytest.raises(DatureConfigError) as excinfo:
        SQLAlchemyConfigLoader(sqlalchemy_source_stub(DB_POOL_RECYCLE="0")).load()
...
def test_rejects_pool_size_outside_range(pool_size: str) -> None:
    with pytest.raises(DatureConfigError) as excinfo:
        SQLAlchemyConfigLoader(sqlalchemy_source_stub(DB_POOL_SIZE=pool_size)).load()
...
def test_rejects_negative_max_overflow() -> None:
    with pytest.raises(DatureConfigError) as excinfo:
        SQLAlchemyConfigLoader(sqlalchemy_source_stub(DB_POOL_MAX_OVERFLOW="-1")).load()
